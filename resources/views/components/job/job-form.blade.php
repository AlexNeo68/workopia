@props(['job' => new \App\Models\Job(), 'do' => 'store'])
...
<form method="POST" action="{{ $route }}" enctype="multipart/form-data">
    ...
    @csrf
    @method($do == 'update' ? 'PUT' : 'POST')

    <h2 class="text-2xl font-bold mb-6 text-center text-gray-500">
        Job Info
    </h2>

    <x-inputs.text name="title" label="Job Title" placeholder="Software Engineer" value="{{ $job->title }}" />

    <x-inputs.text-area id="description" name="description" label="Job Description" value="{{ $job->description }}"
    ...
    <x-inputs.text-area id="benefits" name="benefits" label="Benefits" value="{{ $job->benefits }}"
        placeholder="Health insurance, 401k, paid time off" />

    <x-inputs.text name="tags" value="{{ $job->tags }}" label="Tags (comma-separated)"
        placeholder="development,coding,java,python" />

    <x-inputs.select id="job_type" name="job_type" label="Job Type" value="{{ $job->job_type }}" :options="[
        'Full-Time' => 'Full-Time',
        'Part-Time' => 'Part-Time',
        'Contract' => 'Contract',
        'Temporary' => 'Temporary',
    ...
    <x-inputs.select id="remote" name="remote" value="{{ $job->remote }}" label="Remote" :options="[
        0 => 'No',
        1 => 'Yes',
    ]" />
    ...
    <x-inputs.text name="address" id="address" value="{{ $job->address }}" label="Address"
        placeholder="123 Main St" />

    <x-inputs.text name="city" id="city" value="{{ $job->city ? $job->city : auth()->user()->city }}"
        label="City" placeholder="Albanyt" />

    <x-inputs.text name="coordinates" value="{{ $job->coordinates }}" id="coordinates" label="Coordinates"
    ...
    <x-inputs.text-area id="company_description" value="{{ $job->company_description }}" name="company_description"
        label="Company Description" placeholder="Company Description" />

    <x-inputs.text name="company_website" value="{{ $job->company_website }}" type="url" label="Company Website"
    ...
    <x-inputs.file name="company_logo" value="{{ $job->company_logo }}" id="company_logo" label="Company Logo" />
